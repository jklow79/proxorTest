...
public class SpreadSheet extends JFrame {
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if(tok.length()>2 && tok.substring(0,3).equalsIgnoreCase("AVG")){//handle avg
                
                String startSumPt=tok.substring(4);
                
                String c=tokens.nextToken();//retrieve :
                
                String endSumPt=tokens.nextToken();
                
                if(endSumPt!=null && endSumPt.length()==3){
                    
                    endSumPt=endSumPt.substring(0,2);
                }else{
                    
                    return null;
                }
                      // System.out.println(endSumPt);
                //check next token not exist??
                
                //System.out.println(startSumPt+" "+endSumPt);
                if(startSumPt.charAt(0)==endSumPt.charAt(0)){//same row
                    int num1=Integer.parseInt(startSumPt.substring(1));
                    int num2=Integer.parseInt(endSumPt.substring(1));
                    double sum=0;                    
                    int start=0;
                    int end=0;
                    if(num1>num2){
                    ...
                    String startCol=startSumPt.substring(0,1);
                    for(int i=start;i<=end;i++){
                        //System.out.println(startCol+i);
                        String tok3 = evaluateToken(startCol+i, depth);
                        
                        if (tok3 == null) return null;
                        
                        sum+=Double.parseDouble(tok3);
                ...
                }else if(startSumPt.charAt(1)==endSumPt.charAt(1)){//same col
                    
                    char start0=startSumPt.charAt(0);
                    char end0=endSumPt.charAt(0);
                    ...
                    for(char i=start0;i<=end0;i++){
                        
                         String tok3 = evaluateToken(i+""+startSumPt.charAt(1), depth);
            ...
            try {
                if (depth <= maxRows * maxCols) {
                    StringTokenizer tokens = 
                            new StringTokenizer(formula, "=+*/-:", true); //add :
                    if (tokens.hasMoreTokens() && 
